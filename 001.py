...
For the model, starting with the prior, we first group the data by label and record
their indices by classes:
...
With prior calculated, we continue with likelihood, which is the conditional
...
We set the smoothing value to 1 here, which can also be 0 for no smoothing, or any
other positive value, as long as a higher classification performance is achieved:
>>> smoothing = 1
...
With prior and likelihood ready, we can now compute the posterior for the testing/
new samples:
